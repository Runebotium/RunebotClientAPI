...
public class PathMapOverlay extends Overlay {
    ...
    @Inject
    private PathMapOverlay(Client client, RuneBotWalkerPlugin plugin, RuneBotWalkerConfig config) {
        this.client = client;
        this.plugin = plugin;
        this.config = config;
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        if (plugin.getPathfinder() != null) {
            ...
            for (int i = 0; i < path.size(); i++) {
                ...
                WorldPoint point = WorldPointUtil.unpackWorldPoint(path.get(i).packedPosition);
                WorldPoint last = (i > 0) ? WorldPointUtil.unpackWorldPoint(path.get(i - 1).packedPosition) : point;
