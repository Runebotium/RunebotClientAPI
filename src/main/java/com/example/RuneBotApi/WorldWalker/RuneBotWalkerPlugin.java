...
import com.example.RuneBotApi.WorldWalker.pathfinder.*;
import com.example.RuneBotApi.WorldWalker.walker.WalkerSetTarget;
import com.example.RuneBotApi.WorldWalker.walker.WorldWalker;
...
@PluginDescriptor(
    name = "RB World Walker",
    description = "Fork of Skretzo's ShortestPath Plugin that automates navigating the world."
)
public class RuneBotWalkerPlugin extends Plugin {
    protected static final String CONFIG_GROUP = "RBWalker";
    ...
    private static final BufferedImage MARKER_IMAGE = ImageUtil.loadImageResource(GlobalCollisionMap.class, "marker.png");
    ...
    @Inject
    private RuneBotWalkerConfig config;
    ...
    @Provides
    public RuneBotWalkerConfig provideConfig(ConfigManager configManager) {
        return configManager.getConfig(RuneBotWalkerConfig.class);
    }
    ...
    protected void startUp() {
        ...
        pathfinderConfig = new PathfinderConfig(map, transports, client, this, config, clientThread);
        ...
        eventBus.register(WalkerSetTarget.class);
    }
    ...
    protected void shutDown() {
        ...
        eventBus.unregister(WalkerSetTarget.class);
    }
...
//                pathfinderFuture = pathfindingExecutor.submit(pathfinder);
    ...
    public boolean isNearPath(WorldPoint location) {
        ...
        for (Node node : pathfinder.getPath()) {
            if (location.distanceTo2D(WorldPointUtil.unpackWorldPoint(node.packedPosition)) < config.recalculateDistance()) {
                return true;
            }
        }
    ...
    @Subscribe
    private void onWalkerSetTarget(WalkerSetTarget event) {
        this.setTarget(event.getDestination());
        if (event.getDestination() != null) {
            EthanApiPlugin.sendClientMessage("Walking to: " + event.getDestination());
        }
    }
    ...
    public void onGameTick(GameTick tick) {
        ...
        if (pathfinder != null) {
            if (localPlayer.getWorldLocation().distanceTo(marker.getWorldPoint()) <= 2) {
            ...
            if (walker.getFailedAttempts() >= 10) {
                pathfinder.getPath().clear();
                pathfinder.cancel();
                return;
            }

            walker.update(pathfinder);
        }
    }
        ...
        if (client.isKeyPressed(KeyCode.KC_SHIFT) && event.getOption().equals(WALK_HERE) && event.getTarget().isEmpty()) {
            ...
            if (pathfinder != null) {
                ...
                if (pathfinder.getPath() != null) {
                    for (Node node : pathfinder.getPath()) {
                        if (WorldPointUtil.unpackWorldPoint(node.packedPosition).equals(selectedTile)) {
    ...
    private void onMenuOptionClicked(MenuEntry entry) {
        ...
        if (entry.getOption().equals(SET) && entry.getTarget().equals(TARGET)) {
            WorldWalker.setTarget(getSelectedWorldPoint());
        }
