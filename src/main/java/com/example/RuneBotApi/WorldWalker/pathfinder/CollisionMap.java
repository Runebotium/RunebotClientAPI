    ...
    public List<Node> getNeighbors(Node node, VisitedTiles visited, PathfinderConfig config) {
        ...
        for (int i = 0; i < transports.size(); ++i) {
            ...
            switch (transport.getTransportType()) {
                case CHARTER_SHIP:
                    transportNode = new CharterShipNode(transport, node);
                    break;
                case GNOME_GLIDER:
                    transportNode = new GliderTransportNode(transport, node);
                    break;
                case TELEPORTATION_ITEM:
                    transportNode = new ItemTransportNode(transport, node);
                    break;
                case SPIRIT_TREE:
                    transportNode = new SpiritTreeNode(transport, node);
                    break;
                case FAIRY_RING:
                    transportNode = new FairyRingNode(transport, node);
                    break;
                case TELEPORTATION_SPELL:
                    String spellName = transport.getInteractableName();
                    String actionName = "Cast"; // TODO: Add variety teleport options like Grand Exchange on Varrock
                    WorldArea destinationArea = transport.getDestinationArea();
                    transportNode = new SpellTransportNode(transport, node, spellName, actionName, destinationArea);
                    break;
                default:
                    transportNode = new TransportNode(transport, node);
                    break;
            }
