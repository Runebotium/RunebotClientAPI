...
public class PathMapTooltipOverlay extends Overlay {
    ...
    @Inject
    private PathMapTooltipOverlay(Client client, RuneBotWalkerPlugin plugin, RuneBotWalkerConfig config) {
        this.client = client;
        this.plugin = plugin;
        this.config = config;
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        if (plugin.getPathfinder() != null) {
            ...
            for (int i = 0; i < path.size(); i++) {
                ...
                if(path.size() > i+1){
                    nextPoint = WorldPointUtil.unpackWorldPoint(path.get(i+1).packedPosition);
                }
                if (drawTooltip(graphics, cursorPos, WorldPointUtil.unpackWorldPoint(path.get(i).packedPosition), nextPoint, i + 1)) {
                    return null;
                }
            }
        }
