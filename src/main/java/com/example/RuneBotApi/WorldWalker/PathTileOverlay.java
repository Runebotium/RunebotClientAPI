...
import com.example.RuneBotApi.WorldWalker.pathfinder.CollisionMap;
...
public class PathTileOverlay extends Overlay {
    ...
    @Inject
    public PathTileOverlay(Client client, RuneBotWalkerPlugin plugin, RuneBotWalkerConfig config) {
        this.client = client;
        this.plugin = plugin;
        this.config = config;
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        if (config.drawTiles() && plugin.getPathfinder() != null && plugin.getPathfinder().getPath() != null) {
            ...
            List<Node> path = plugin.getPathfinder().getPath();
            int counter = 0;
            if (TileStyle.LINES.equals(config.pathStyle())) {
                for (int i = 1; i < path.size(); i++) {
                    WorldPoint previous = WorldPointUtil.unpackWorldPoint(path.get(i - 1).packedPosition);
                    WorldPoint current = WorldPointUtil.unpackWorldPoint(path.get(i).packedPosition);
                    drawLine(graphics, previous, current, color, 1 + counter++);
                    drawTransportInfo(graphics, previous, current);
                }
            } else {
                ...
                for (int i = 0; i <  path.size(); i++) {
                    ...
                    if (i + 1 < path.size()) {
                        next = WorldPointUtil.unpackWorldPoint(path.get(i + 1).packedPosition);
                    }
                    drawTile(graphics, current, color, counter++, showTiles);
                    drawTransportInfo(graphics, current, next);
                }
            }
        }
            ...
            for (WorldPoint pointEnd : WorldPoint.toLocalInstance(client, locationEnd))
            {
                ...
                for (Transport transport : plugin.getTransports().getOrDefault(point, new ArrayList<>())) {
                    ...
                    if (transport.getDestination() == null) {
                        dest = transport.getDestinationArea().toWorldPoint();
                    } else {
                        dest = transport.getDestination();
                    }

                    if (!pointEnd.equals(dest)) {
                        continue;
                    }
